...
from utils.affinity_stmap import multi_offset, gen_affs_stmap
...
class Train(Dataset): 
    def __init__(self, cfg, mode='train'): 
        ...
        self.max_neighbor = cfg.DATA.obj_neighbor_max 
        ...
        self.id_img.sort(key=lambda x: int(x.split("_")[0]))
        self.id_label.sort(key=lambda x: int(x.split("_")[0]))
        self.id_fg.sort(key=lambda x: int(x.split("_")[0]))
    ...
    def __getitem__(self, idx):
        ...
        if self.padding:
            ...
            data = np.pad(data, ((7,7),(22,22),(0,0)), mode='constant')
            data = Image.fromarray(data).resize((self.size, self.size),Image.ANTIALIAS)
            label = np.asarray(label)
            label = np.pad(label, ((7,7),(22,22)), mode='constant')
            label = Image.fromarray(label).resize((self.size, self.size),Image.ANTIALIAS)
        ...
        lb_affs, affs_mask, neighbor_mask = gen_affs_stmap(label_numpy, offsets=self.offsets, max_neighbor = self.max_neighbor, ignore=False, padding=True)
        lb_affs1, affs_mask1, neighbor_mask1 = gen_affs_stmap(label_down1, offsets=self.offsets[:self.nb_half*4], max_neighbor = self.max_neighbor, ignore=False, padding=True)
        lb_affs2, affs_mask2, neighbor_mask2 = gen_affs_stmap(label_down2, offsets=self.offsets[:self.nb_half*3], max_neighbor = self.max_neighbor, ignore=False, padding=True)
        lb_affs3, affs_mask3, neighbor_mask3 = gen_affs_stmap(label_down3, offsets=self.offsets[:self.nb_half*2], max_neighbor = self.max_neighbor, ignore=False, padding=True)
        lb_affs4, affs_mask4, neighbor_mask4 = gen_affs_stmap(label_down4, offsets=self.offsets[:self.nb_half*1], max_neighbor = self.max_neighbor, ignore=False, padding=True)
        # lb_affs, affs_mask = gen_affs_stmap(label_numpy, offsets=self.offsets, ignore=False, padding=True)
        # print(lb_affs.shape, affs_mask.shape, neighbor_mask.shape)
...
class Validation(Train):
    ...
    def __getitem__(self, k):
        data = Image.open(os.path.join(self.dir, self.id_img[k])).convert('RGB')
        if self.mode == 'test':
            label = Image.open(os.path.join(self.dir, self.id_fg[k])).convert('L')
        else:
            label = Image.open(os.path.join(self.dir, self.id_label[k]))
...
def show_batch(temp_data, out_path):
    ...
    im_cat = np.concatenate([tmp_data, seg_color], axis=1)
